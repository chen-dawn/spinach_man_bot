...
def slack_events():
    try:
        data = request.json
        logger.info(
            f"Received event: {json.dumps(data)[:200]}..."
        )  # Log truncated event data
        ...
        if "event" in data:
            ...
            if event.get("subtype") == "message_changed" or event.get("bot_id"):
                logger.info(
                    f"Skipping message: subtype={event.get('subtype')}, bot_id={event.get('bot_id')}"
                )
                ...
                if not has_url and "blocks" in event:
                    ...
                    for block in blocks:
                                ...
                                if element.get("type") == "rich_text_section":
                                    for item in element.get("elements", []):
                                        if item.get("type") == "link" and "url" in item:
                                            has_url = True
                                            logger.info(
                                                f"Found URL in rich_text block: {item.get('url')}"
                                            )
                                            break
                        ...
                        # Handle section blocks with text
                            ...
                            if isinstance(text_obj, dict) and "http" in text_obj.get(
                                "text", ""
                            ):
                                has_url = True
                                logger.info(
                                    f"Found URL in section block: {text_obj.get('text')[:50]}..."
                                )
...
def handle_message(event):
    try:
        ...
        thread_ts = event.get(
            "thread_ts", None
        )  # Get thread timestamp if message is in a thread
    ...
    try:
        ...
        if response.status_code != 200:
            if response.status_code == 403 and "www.cell.com" in url:
                ...
                return (
                    f"Unable to access the webpage. Status code: {response.status_code}"
                )
        ...
        response = openai_client.responses.create(
            ...
            instructions='''
                ...
                Provide a concise, insightful summary of the following academic paper that balances technical accuracy 
